...
class MetersPredictor:
    def __init__(self, segmentation_model_file, faster_rcnn_path):
        self.segmentation_model = load_segmentation_model(segmentation_model_file)
        self.predictor = configure_predictor(faster_rcnn_path)

    def predict(self, image):
        # image = read_cv2_image(image)
        # image = "IMG_20221109_223004.jpg"
        return get_reading(image, self.segmentation_model, self.predictor)


def read_cv2_image(binaryimg):
    stream = standard_io.BytesIO(binaryimg)
...
def load_segmentation_model(segmentation_model_file):
    return tf.keras.models.load_model(segmentation_model_file, custom_objects={
        'binary_crossentropy_plus_jaccard_loss': sm.losses.bce_jaccard_loss, 'iou_score': sm.metrics.iou_score})


def configure_predictor(faster_rcnn_path):
    # Configure model parameters.
    cfg = get_cfg()
    cfg.merge_from_file(model_zoo.get_config_file("COCO-Detection/faster_rcnn_X_101_32x8d_FPN_3x.yaml"))
    cfg.MODEL.WEIGHTS = faster_rcnn_path
...
def extract_meter(image_to_be_cropped):
    ...
    '''
    # cv2.imshow("", image_to_be_cropped)
    # cv2.waitKey(0)
...
def prep_for_ocr(img):
    ...
    output_name = 'tmp/test_img_for_ocr.jpg'
    cv2.imwrite(output_name, img)
    return output_name


# Segment input image.
def segment_input_img(img, model):
    # Resize image.
...
def get_reading(image, model, predictor):
    '''
    ...
    # Segment image.
    segmented = segment_input_img(image, model)
